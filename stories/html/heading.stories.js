export default {
  title: 'HTML/ContentSectioning'
};

export const address = () => '<address>Address</address>';
export const article = () => '<article>Article</article>';
export const aside = () => '<aside>Aside</aside>';
export const footer = () => '<footer>Footer</footer>';
export const header = () => '<header>Header</header>';
export const allHeadings = () => '<h1>Heading 1</h1> <h2>Heading 2</h2> <h3>Heading 3</h3> <h4>Heading 4</h4> <h5>Heading 5</h5> <h6>Heading 6</h6>';
export const H1 = () => '<h1>Heading 1</h1>';
export const H2 = () => '<h2>Heading 2</h2>';
export const H3 = () => '<h3>Heading 3</h3>';
export const H4 = () => '<h4>Heading 4</h4>';
export const H5 = () => '<h5>Heading 5</h5>';
export const H6 = () => '<h6>Heading 6</h6>';
export const hgroup = () => '<hgroup>HGroup</hgroup>';
export const main = () => '<main>Main</main>';
export const nav = () => '<nav>Nav</nav>';
export const section = () => '<section>Section</section>';
export const search = () => '<search>Search</search>';
